...
class TestTranslationAPI(TestCase):
    ...
    def test_asr_end_point(self):
        ...
        with mock.patch(
            "bhashini_translator.bhashini_translator.Bhashini.asr"
        ) as mock_requests:
            self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    ...
    def test_auth_required_nmt_tts(self):
        response = self.client.post(
            "/api/nmt_tts/",
            {
                "sourceLanguage": "en",
    ...
    def test_nmt_tts_end_point(self):
        response = self.client.post(
            "/api/token/", data={"username": "testuser", "password": "password"}
        )
        token = response.data.get("access")
        ...
        with mock.patch(
            "bhashini_translator.bhashini_translator.Bhashini.nmt_tts"
        ) as mock_main:
            self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    ...
    def test_auth_required_asr_nmt_tts(self):
        response = self.client.post(
            "/api/asr_nmt_tts/",
            {
                "sourceLanguage": "en",
